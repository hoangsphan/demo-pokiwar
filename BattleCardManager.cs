...
public class BattleCardManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (GameSession.Instance != null && GameSession.Instance.selectedCardPrefabs.Count > 0)
        {
            // Lấy danh sách thẻ đã chọn từ GameSession
            var cardsToLoad = GameSession.Instance.selectedCardPrefabs;
            SpawnCards(cardsToLoad);
        }
        else
        {
            Debug.LogWarning("[BattleCardManager] Không tìm thấy GameSession hoặc không có thẻ nào được chọn.");
        }
    }
