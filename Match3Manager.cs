...
public class Match3Manager : MonoBehaviour
{
    ...
    // Cho BattleManager gọi để ẩn bàn cờ
    public void HideGrid()
    {
        if (gridParent) gridParent.gameObject.SetActive(false);
        if (cursorHighlight) cursorHighlight.SetActive(false); // Ẩn luôn cursor
    }

    // Cho BattleManager gọi để hiện lại bàn cờ
    public void ShowGrid()
    {
        if (gridParent) gridParent.gameObject.SetActive(true);
        if (cursorHighlight) cursorHighlight.SetActive(true);
    }
}
