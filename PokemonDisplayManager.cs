...
public class PokemonDisplayManager : MonoBehaviour
{
    ...
    GameObject SetupCharacter(bool isPlayer)
    {
        ...
        {
            pos = playerPos;
            // 1. SỬA: Thử lấy prefab từ GameSession
            if (GameSession.Instance != null && GameSession.Instance.selectedPetPrefab != null)
            {
                prefabToSpawn = GameSession.Instance.selectedPetPrefab;
                Debug.Log("[PokemonDisplayManager] Đã tải Player Pet từ GameSession: " + prefabToSpawn.name);
            }
            // 2. Nếu không có (ví dụ: test scene trực tiếp)
            else
            {
                Debug.LogWarning("[PokemonDisplayManager] Không tìm thấy GameSession.Instance hoặc selectedPetPrefab. Sử dụng prefab PLAYER MẶC ĐỊNH.");
