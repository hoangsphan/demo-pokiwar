...
public class LobbyManager : MonoBehaviour
{
    ...
    [Header("Data Lựa chọn (SẼ LƯU VÀO GAMESESSION)")]
    ...
    void Start()
    {
        ...
        if (backToMapButton != null) backToMapButton.onClick.AddListener(GoBackToMap);
    ...
    void LoadDataFromDatabase()
    {
        ...
        // Tải Pet đã trang bị từ Database
        ...
        // Tải các Thẻ đã trang bị từ Database
    ...
    public void OnPlayClicked()
    {
        ...
        {
            GameSession.Instance.selectedPetPrefab = this.selectedPetPrefab;
            GameSession.Instance.selectedCardPrefabs = new List<GameObject>(this.selectedCardPrefabs);
            Debug.Log("Đã lưu lựa chọn (Pet/Card) vào GameSession.");
        }
        else
    ...
    {
        if (GameSession.Instance != null && !string.IsNullOrEmpty(GameSession.Instance.previousSceneName))
        {
            // Lấy tên scene đã lưu (ví dụ: "FireMap" hoặc "WaterMap")
            string mapToLoad = GameSession.Instance.previousSceneName;

            // Dọn dẹp lựa chọn enemy hiện tại
            GameSession.Instance.selectedEnemyPrefab = null;
            GameSession.Instance.previousSceneName = null; // Xóa luôn scene name cũ
        ...
        {
            // Phòng trường hợp chạy LobbyScene trực tiếp (chưa có data GameSession)
            Debug.LogWarning("Không tìm thấy previousSceneName, quay về MenuPB (Màn hình chọn đảo)");
            SceneManager.LoadScene("MenuPB"); // Quay về màn hình chọn đảo chính
        }
    }
}